...
public class q3 implements Runnable {
    ...
    public void solve() {
        ...
        for (int i = 0; i < n; i++) {
            memo.put((int)arr[i], memo.getOrDefault((int)arr[i], 0) + 1);
        }
        for (int i = 0; i < n; i++) {
            if (!memo.containsKey((int)arr[i])) continue;
            int a = (int)arr[i];
            double temp = ((2 * ((double)(sum - (double)a))) - ((n - 2) * (double)a)) / (n);
            if (temp >= 0 && (int)Math.ceil(temp) == (int)temp) {
                int temp2 = (int)temp;
                if (memo.containsKey(temp2)) {
                    if (temp2 == a) {
                        if (memo.get(a) > 1) {
                            count += ((((long)memo.get(a)) * ((long)(memo.get(a) - 1l))) / 2l);
                            memo.remove(a);
                        }
                        continue;
                    }
                    count += (((long)memo.get(temp2)) * ((long)memo.get(a)));
                    memo.remove(temp2);
                    memo.remove(a);
                }
            }
