...
public class q1 implements Runnable {
    ...
    private int n, inf = 1_000_000_001;
    ...
    public void solve() {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                for (int q = 0; q < 20; q++) dp[i][j][q] = inf;
            }
        }
        long temp = helper(0, 0, 1);
        for (int i = 2; i < 10; i++) {
            temp = Math.min(temp, helper(0, 0, i));
        }
        pw.println(temp);
    ...
    private long helper(int x, int y, int demand) {
        if (x == n - 1 && y == n - 1) {
            long[] temp = calculate(1, x, y);
            dp[x][y][(int)temp[1]] = temp[0];
            return dp[x][y][demand];
        }
        if (dp[x][y][demand] != inf) {
            // System.out.println("Here");
            // System.out.println("x=" + x + " , y=" + y + " , demand=" + demand);
            // System.out.println(dp[x][y][demand]);
            return dp[x][y][demand];
        }
        ...
        if (x + 1 < n) {
            for (int i = 1; i < 10; i++) {
                long temp = helper(x + 1, y, i);
                if (temp < inf) {
                    long[] temp2 = calculate(i, x, y);
                    if (dp[x][y][(int)temp2[1]] > temp2[0] + temp) {
                        dp[x][y][(int)temp2[1]] = temp2[0] + temp;
                        dp[x][y][(int)temp2[1] + 10] = 1;
                    }
                } else {
                    dp[x + 1][y][i] = inf + 1;
                }
        ...
        if (y + 1 < n) {
            for (int i = 1; i < 10; i++) {
                long temp = helper(x, y + 1, i);
                if (temp < inf) {
                    long[] temp2 = calculate(i, x, y);
                    if (dp[x][y][(int)temp2[1]] > temp2[0] + temp) {
                        dp[x][y][(int)temp2[1]] = temp2[0] + temp;
                        dp[x][y][(int)temp2[1] + 10] = 0;
                    }
                } else {
                    dp[x][y + 1][i] = inf + 1;
                }
    ...
    private long[] calculate(long prev, int x, int y) {
        long count = 0;
        long temp = arr[x][y] * prev;
        while (temp % 10 == 0) {
            count++;
            temp /= 10;
        }
        temp %= 10;
        return new long[] {count, temp};
    }
