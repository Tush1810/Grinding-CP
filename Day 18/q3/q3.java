...
//
    ...
    public void solve() {
        ...
        dp = new long[n + 1][countFors + 2];
        for (long[] temp : dp) Arrays.fill(temp, -1);
        pw.println(helper(0, 0));
    }

    private long helper(int pos, int indentationLevel) {
        if (pos == n) {
            return 1;
        }
        if (dp[pos][indentationLevel] != -1) return dp[pos][indentationLevel];
        ans = 0;
        if (arr[pos] == 1) {
            if (pos >= 1 && arr[pos - 1] == 1) {
                ans += helper(pos + 1, indentationLevel + 1);
            } else {
                if (indentationLevel != 0) {
                    ans += helper(pos, indentationLevel - 1);
                }
                ans += helper(pos + 1, indentationLevel + 1);
            }
        } else {
            if (pos >= 1 && arr[pos - 1] == 1) {
                ans += helper(pos + 1, indentationLevel);
            } else {
                if (indentationLevel != 0) {
                    ans += helper(pos, indentationLevel - 1);
                }
        ...
        if (ans >= mod) ans %= mod;
        return dp[pos][indentationLevel] = ans;
    }
