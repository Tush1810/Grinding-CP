...
public class q2 implements Runnable {
    ...
    public void solve() {
        ...
        pallindrome = new boolean[n + 1][n + 1];
        for (int length = 1; length <= n; length++) {
            for (int i = 0; i + length - 1 < n; i++) {
                if (arr[i] == arr[i + length - 1] && ((i + 1 > i + length - 2) || pallindrome[i + 1][i + length - 2])) {
                    pallindrome[i][i + length - 1] = true;
                }
            }
        }
        ...
        while (q-- > 0) {
            pw.println(dp[sc.nextInt() - 1][sc.nextInt() - 1]);
        }
    }
    ...
    private int helper(char[] arr, int start, int end) {
        ...
        return dp[start][end] = (helper(arr, start, end - 1) + helper(arr, start + 1, end) - helper(arr, start + 1, end - 1) + (pallindrome[start][end] ? 1 : 0));
    }
