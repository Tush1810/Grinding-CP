...
public class q1 implements Runnable {
    ...
    public void solve() {
        ...
        for (int i = 1; i < n; i++) {
            ...
            dp[i] = temp;
            if (i >= k) {
                dp[i] = Math.min(dp[i], ((arr[i] == 0) ? 1 : 0) + dp[i - k] + pref[i - 1] - pref[i - k]);
            }
        }
        for (int i = 0; i < n; i++) {
            // pw.println("i=" + i + " , ans=" + ans + " , dp[" + i + "]=" + dp[i]);
            ans = Math.min(ans, dp[i] + pref[n - 1] - pref[i]);
        }
        // pw.println("i=" + n + " , ans=" + ans);
        pw.println(ans);
    }
