...
public class q3_original_approach implements Runnable {

    public static void main(String[] args) {
        new Thread(null, new q3_original_approach(), "whatever", 1 << 26).start();
    }
    ...
    public void solve() {
        ...
        if (countEvens >= countOdds) {
            ...
            ArrayDeque<Integer> oddPos = new ArrayDeque<Integer>();
            for (int i = 1; i < n - 1; i++) {
                if (arr[i] % 2 == 1) {
                    if (arr[i] == 1) {
                        oddPos.addLast(i);
            ...
            long tempAns = 0;
            boolean found = false;
            while (countEvens < countOdds && oddPos.size() >= 2) {
                int temp1 = oddPos.poll();
                if (arr[temp1] == 1) {found = true; break;}
                int temp2 = oddPos.pollLast();
                arr[temp2]++;
                arr[temp1] -= 2;
                if (arr[temp1] == 1) {
                    oddPos.addLast(temp1);
                } else {
                    oddPos.addFirst(temp1);
                }
                tempAns++;
                countEvens += arr[temp2] / 2;
                countOdds--;
            }
            if (found) {
                printer(arr, tempAns);
