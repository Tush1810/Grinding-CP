...
public class q3 implements Runnable {
    ...
    public void solve() {
        ...
        ArrayList<Long>[] suffix = new ArrayList[n];
        for (int i = 0; i < n; i++) {
            arr[i] = new ArrayList<Long>();
            suffix[i] = new ArrayList<Long>();
        }
        int[] uni = sc.nextIntArray(n);
        for (int i = 0; i < n; i++) {
            uni[i]--;
            int skill = sc.nextInt();
            arr[uni[i]].add((long)skill);
        }
        for (int i = 0; i < n; i++) {
            Sorter.sort(arr[i]);
        }
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 1; j < arr[i].size(); j++) {
                arr[i].set(j, arr[i].get(j - 1) + arr[i].get(j));
            }
        }
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 1; j <= arr[i].size(); j++) {
                if (arr[i].size() % j == 0) {
                    ans[j] += arr[i].get(arr[i].size() - 1);
                } else
                    ans[j] += arr[i].get(arr[i].size() - 1) - arr[i].get((arr[i].size() % j) - 1);
            }
        }
