...
public class q1 implements Runnable {
    ...
    public void solve() {
        ...
        long sum = getDistance(positives, k) + getDistance(negatives, k);
        if (positives.size() == 0) {
            sum -= negatives.get(negatives.size() - 1);
        } else if (negatives.size() == 0) {
            sum -= positives.get(positives.size() - 1);
        } else if (positives.get(positives.size() - 1) >= negatives.get(negatives.size() - 1)) {
            sum -= positives.get(positives.size() - 1);
        } else {
            sum -= negatives.get(negatives.size() - 1);
        }
    ...
    private long getDistance(ArrayList<Long> arr, int k) {
        long sum = 0;
        int n = arr.size();
