...
public class q1 implements Runnable {
    ...
    public void solve() {
        ...
        int[][] divisors = new int[1_000_001][];
        for (int i = 0; i < n; i++) {
            getDivisors((int)arr[i], divisors);
        }
        dp = new long[n + 1];
        dp[0] = 1;
        for (int i = 0; i < n; i++) {
            for (int x : divisors[(int)arr[i]]) {
                if (x > n) continue;
                dp[x] = modAdd(dp[x], dp[x - 1]);
            }
        }
        dp[0] = 0;
        for (int i = 1; i <= n; i++) {
    ...
    private void getDivisors(int x, int[][] arr) {
        int count = 0;
        for (int i = 1; i * i <= x; i++) {
            if (x % i == 0) {
                count++;
        ...
        for (int i = 1; i * i <= x; i++) {
            if (x % i == 0) {
                arr[x][count - 1 - (count3++)] = i;
                if ((x / i) != i) {
                    arr[x][count2++] = (int)(x / i);
                }
            }
        }
    }
