...
public class q1 implements Runnable {
    ...
    public void solve() {
        ...
        for (int i = 0; i < n - 1; i++) isPalindrome[i][i + 1] = (arr[i] == arr[i + 1]);
        for (int i = 3; i <= n; i++) {
    ...
    private int helper(int[] arr, int start, int end) {
        ...
        if (isPalindrome[start][end]) return dp[start][end] = 1;
        if (dp[start][end] != Integer.MAX_VALUE) return dp[start][end];
        for (int j = end; j > start; j--) {
            dp[start][end] = Math.min(dp[start][end], helper(arr, start, j - 1) + helper(arr, j, end));
            if (arr[start] == arr[j] && j > start + 1) {
                dp[start][end] = Math.min(dp[start][end], helper(arr, start + 1, j - 1) + helper(arr, j + 1, end));
            }
        }
        if (arr[start] == arr[start + 1] && start + 2 <= end) {
            dp[start][end] = Math.min(dp[start][end], dp[start + 2][end] + 1);
        }
        return dp[start][end];
    }
