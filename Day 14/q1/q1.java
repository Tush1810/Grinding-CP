...
public class q1 implements Runnable {
    ...
    private class Robot {
        ...
        Robot(int x, int y, int f0, int f1, int f2, int f3) {
            this.x = x;
            this.y = y;
            this.f0 = f0 == 1 ? true : false;
            this.f1 = f1 == 1 ? true : false;
            this.f2 = f2 == 1 ? true : false;
            this.f3 = f3 == 1 ? true : false;
            this.minx = x;
            this.miny = y;
            this.maxx = x;
            this.maxy = y;
            if (this.f0) minx = -100_000;
            if (this.f1) maxy = 100_000;
            if (this.f2) maxx = 100_000;
            if (this.f3) miny = -100_000;
        }
        // f0 = go left
        // f1 = go up
        // f2 = go right
    ...
    public void solve() {
        ...
        for (int i = 0; i < n; i++) {
            arr[i] = new Robot(sc.nextInt(), sc.nextInt(), sc.nextInt(), sc.nextInt(), sc.nextInt(), sc.nextInt());
        }
        int minX = arr[0].minx, minY = arr[0].miny, maxX = arr[0].maxx, maxY = arr[0].maxy;
        for (int i = 0; i < n; i++) {
            minX = Math.max(arr[i].minx, minX);
            minY = Math.max(arr[i].miny, minY);
            maxX = Math.min(arr[i].maxx, maxX);
            maxY = Math.min(arr[i].maxy, maxY);
        }
        if (minX <= maxX && minY <= maxY) {
            pw.println(1 + " " + minX + " " + minY);
        } else {
            pw.println(0);
        }
    }
