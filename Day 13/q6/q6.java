...
public class q6 implements Runnable {
    ...
    public static void main(String[] args) {
        new Thread(null, new q6(), "whatever", 1 << 26).start();
    }
    ...
    public void solve() {
        ...
        if (sum != 0) {
            ...
            dq.addLast(new Pair(dq.pollLast().first, n - 1));
        }

        if (dq.size() < k || ((dq.size() - k) % 2 == 1)) {
            pw.println("NO");
            return;
        }
        pw.println("YES");
        ...
        while (true) {
            if (count == k - 1) {
                pw.print(dq.pollLast().second + "\n");
                break;
            } else {
                pw.print(dq.pollFirst().second + " ");
            }
            count++;
        }
    }
