...
public class q2 implements Runnable {
    ...
    public void solve() {
        ...
        for (int i = 0; i < n; i++) memo.put(arr[i], i);
        Node root = helper(0, n - 1, arr, memo);
        int[] ans = new int[n];
        Node temp = root;
        dfs(ans, temp, 0);
        for (int i = 0; i < n; i++) {
    ...
    private Node helper(int start, int end, int[] arr, HashMap<Integer, Integer> memo) {
        ...
        Node root = new Node(max, index);
        root.left = helper(start, index - 1, arr, memo);
        root.right = helper(index + 1, end, arr, memo);
        return root;
    }

    private void dfs(int[] ans, Node root, int height) {
        if (root == null) return;
        ans[root.index] = height;
