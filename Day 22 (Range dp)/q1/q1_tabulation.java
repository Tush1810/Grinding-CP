...
public class q1_tabulation implements Runnable {

    public static void main(String[] args) {
        new Thread(null, new q1_tabulation(), "whatever", 1 << 26).start();
    }
    ...
    public void solve() {
        ...
        for (int i = 0; i < n; i++) arr[i] = new Pair(a[i], sc.nextInt());
        ...
        //     return;
        // }
        // dp = new int[n + 1][k + 1][10_001];
        // for (int i = 0; i <= n; i++) {
        //     for (int j = 0; j <= k; j++) {
        //         Arrays.fill(dp[i][j], -1);
        //     }
        // }
        // pw.println(helper(0, 0, 0));
        int capacity = 10_000;
        dp2 = new int[n + 1][capacity + 1];
        for (int i = 0; i <= n; i++) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = i; j >= 0; j--) {
                for (int q = capacity; q >= 0; q--) {
                    if (dp2[j][q] != inf) {
                        dp2[j + 1][q + arr[i].second] = Math.min(dp2[j][q], dp2[j + 1][q + arr[i].second]);
                        dp2[j][q] += arr[i].first;
                    }
                }
            }
        ...
        for (int j = 0;; j++) {
            int ans = inf;
            for (int q = capacity; q >= sum; q--) {
                ans = Math.min(ans, dp2[j][q]);
            }
            if (ans != inf) {
                pw.println(j + " " + ans);
                break;
    ...
    private int helper(int pos, int count, int capacity) {
        if (pos >= n) {
            if (capacity < sum) {
                return inf;
            } else {
        ...
        if (dp[pos][count][capacity] != -1) return dp[pos][count][capacity];
        dp[pos][count][capacity] = helper(pos + 1, count, capacity) + arr[pos].first;
        if (count < k) {
            dp[pos][count][capacity] = Math.min(dp[pos][count][capacity],
                                                helper(pos + 1, count + 1, capacity + arr[pos].second));
        }
        return dp[pos][count][capacity];
    }
