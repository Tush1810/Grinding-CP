...
public class q4 implements Runnable {
    ...
    public void solve() {
        ...
        int[] arr = new int[n];
        long[] pre = new long[n];
        long[] subArrays = new long[n + 1];
        for (int i = 0; i < n; i++) {
            arr[i] = sc.nextInt();
        }
        Arrays.fill(subArrays, Integer.MIN_VALUE);
        pre[0] = arr[0];
        for (int i = 1; i < n; i++) pre[i] = pre[i - 1] + arr[i];
        for (int q = 1; q <= n; q++) {
            for (int i = 0; i + q - 1 < n; i++) {
                subArrays[q] = Math.max(subArrays[q], pre[i + q - 1] - pre[i] + (long)arr[i]);
            }
            // System.out.println("q=" + q + " , subArrays[q]=" + subArrays[q]);
        }
        for (int q = 0; q < n + 1; q++) {
            long max = 0;
            for (int i = 1; i < q; i++) {
                max = Math.max(max, subArrays[i] + i * k);
            }
            for (int i = q; i <= n; i++) {
                max = Math.max(max, subArrays[i] + (long)(q * (long)k));
            }
            pw.print(max + " ");
        }
        pw.println();
